...
void updategen(int currentgen[], int length)
{
    ...
    int *nextcurrent; /* Initialize the pointer for the new gen */
    int *prev; /* Initialize the pointers for the current gen */
    int *curcurrent;
    int *next;
    int i;
    ...
    nextcurrent = next_gen + 1;
    next_gen[0] = 0;
    next_gen[(length - 1)] = 0;
    prev = currentgen;
    curcurrent = currentgen + 1;
    next = currentgen + 2;
    for (i = 1; i < (length - 1); i++)
    {
        if (*curcurrent == 0 && \
    ...
    nextcurrent = next_gen; /* Reinitialize pointers back to index 0 */
    curcurrent = currentgen;
    for (i = 0; i < length; i++) /* Assign next_gen to currentgen */
    {
        *curcurrent = *nextcurrent;
        nextcurrent++;
        curcurrent++;
    }
